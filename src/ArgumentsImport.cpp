...
{
	_ADD_ARGUMENT("script", "Import field scripts. Possible values: mrs, xml, txt", "script", "");
	_parser.addPositionalArgument("directory", QCoreApplication::translate("ArgumentsImport", "Input directory."));
	_parser.addPositionalArgument("target_file", QCoreApplication::translate("ArgumentsImport", "Output file (optional)."), "[target_file]");

	parse();
}

QString ArgumentsImport::scriptFormat() const
{
	return _parser.value("script");
}

QString ArgumentsImport::directory() const
{
	return _directory;
}
...
{
	return _targetFile.isEmpty() ? _path : _targetFile;
}
